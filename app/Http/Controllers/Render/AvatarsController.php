    ...
    public function render(User $user)
    {
        ...
        if($user->exists() && $user->avatar()->exists())
        {
            ...
            if (!$lockAcquired) {
             ...
             if($user->avatar_render > (Carbon::now()->subSeconds(3)))
             {
                 return response()->json([
                     'success' => 'false',
                     'errors' => [
                         'code' => '0',
                         'message' => 'Something went wrong with that request, see response status code. too fast'
                     ],
            ...
            //    ], 404, [], JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT);
                ...
                $user->avatar_render = Carbon::now();
                $user->save();
                app('App\Http\Controllers\Render\AvatarsController')->headshot($user);
                return;
            //    return response()->json([
            //        'success' => 'true',
            //        'hash' => $randomHash,
