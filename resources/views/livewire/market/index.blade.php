<div>
    <div class="grid-x grid-margin-x" style="height: auto !important;">
        @forelse($items as $item)
            <div wire:key="market-item-{{ $item->id }}" class="cell small-6 medium-2 market-item-cell">
                <a href="{{ route('market.item', $item->id) }}" title="{{ $item->name }}">
                    @if($item->special) <div class="ribbon"><span>Collectible</span></div> @endif
                    <img class="market-item-thumbnail " src="{{ $item->render() }}">
                </a>
        ...
        @empty
        <div class="cell">No items found :(</div>
        @endforelse
    </div>
</div>
